...
import { bufferFromString, ONE_SOL, uuidToBn } from "../tests/util/setup";
...
const SALE_NAME = "Test name";
const SALE_DESCRIPTION = "Test Token description";

const START_DATE = now;
const END_DATE = now.addn(1 * 60);
...
async function main() {
  ...
  logVar(`Creating sale with id`, id.toString());
  logVar(`Pda`, saleAddress.toString());
  logVar(`Contract owner`, authority.toString());
  ...
  return await program.methods
    .participateInSale(id, ONE_SOL, 1)
    .accounts({
      sender: authority,
    })
    .rpc();
